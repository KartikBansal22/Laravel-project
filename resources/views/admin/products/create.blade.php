    ...
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-3 gap-4">
            ...
            <div>
                ...
                <input type="number" name="stock_quantity" value="{{ old('stock_quantity', 0) }}" class="w-full border rounded p-2" required>
            </div>
            <div>
                <label class="block font-semibold mb-1">Reorder Threshold</label>
                <input type="number" name="reorder_threshold" value="{{ old('reorder_threshold', 5) }}" class="w-full border rounded p-2">
            </div>
        </div>
        ...
        <button type="submit" class="bg-slate-800 text-white px-6 py-2 rounded">Create Product</button>
    </form>
</x-layout>